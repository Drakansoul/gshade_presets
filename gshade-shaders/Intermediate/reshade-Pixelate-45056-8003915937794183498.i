...
#line 1 "C:\Program Files\GShade\gshade-shaders\Shaders\Pixelate.fx"
#line 4
...
#line 28 "C:\Program Files\GShade\gshade-shaders\Shaders\Pixelate.fx"
#line 1 "C:\Program Files\GShade\gshade-shaders\Shaders\DrawText.fxh"
#line 174
...
#line 204
float2 uv = (tex * float2(3840, 2137) - pos) / size;
uv.y      = saturate(uv.y);
uv.x     *= ratio * 2.0;
#line 208
...
#line 29 "C:\Program Files\GShade\gshade-shaders\Shaders\Pixelate.fx"
#line 31
void PixelatePass(in float4 position : SV_Position, in float2 texcoord : TEXCOORD, out float4 color : SV_Target)
{
int2 pixcoord = floor((float2(3840, 2137) * texcoord) / cell_size) * cell_size;
color = tex2Dlod(ReShade::BackBuffer, float4(((pixcoord) + 0.5) * float2((1.0 / 3840), (1.0 / 2137)), 0.0, 0.0));
#line 36
if(avg_amount > 0.1)
{
...
for( int x = 0 ; x < 4 ; ++x )
for( int y = 0 ; y < 4 ; ++y )
avg_color += tex2Dlod(ReShade::BackBuffer, float4(((float2(pixcoord.x + (x * step), pixcoord.y + (y * step))) + 0.5) * float2((1.0 / 3840), (1.0 / 2137)), 0.0, 0.0));
#line 45
avg_color *= 0.0625;
...
if(stats)
{
DrawText_Digit(float2(float2(3840, 2137).x - 256, 128), 64, 1, texcoord, -1, (float2(3840, 2137).x / cell_size), color.x);
DrawText_Digit(float2(float2(3840, 2137).x - 256, 192), 64, 1, texcoord, -1, (float2(3840, 2137).y / cell_size), color.x);
}
}
#line 56
technique Pixelate
