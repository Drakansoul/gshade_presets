...
texture Blur0{Width = 3840; Height = 2137; Format = RGB10A2;};
texture Blur1{Width = 3840; Height = 2137; Format = RGB10A2;};
#line 82
...
prev[0] = UnpackColors(tex2Dfetch(sBlur1, clamp(float2(coord.x - filterWidth, coord.y), 0, float2(3840, 2137) - 1))).xyzx;
...
{
float2 sampleCoord = clamp(float2(coord.x + i, coord.y), 0, float2(3840, 2137) - 1);
...
float3 currWeights = float3(3, 6, 3) / denominator;
float2 prevWeights = float2(-6 + 2*k*k, -k*k + 3 * k - 3) / denominator;
prev[0] = UnpackColors(tex2Dfetch(sBlur0, clamp(float2(coord.x + filterWidth, coord.y), 0, float2(3840, 2137) - 1))).xyzx;
prev[1] = prev[0].yyyy;
prev[2] = prev[0].zzzz;
prev[0] = prev[0].xxxx;
curr[0] = prev[0];
...
float2 prevWeights = float2(-6 + 2*k*k, -k*k + 3 * k - 3) / denominator;
#line 284
prev[0] = UnpackColors(tex2Dfetch(sBlur1, clamp(float2(coord.x, coord.y - filterWidth), 0, float2(3840, 2137) - 1))).xyzx;
prev[1] = prev[0].yyyy;
prev[2] = prev[0].zzzz;
prev[0] = prev[0].xxxx;
curr[0] = prev[0];
...
for(int i = -filterWidth + 1; i < 256; i++)
{
float2 sampleCoord = clamp(float2(coord.x, coord.y + i), 0, float2(3840, 2137) - 1);
float3 newSample = UnpackColors(tex2Dfetch(sBlur1, sampleCoord));
[unroll]
for(int j = 0; j < 3; j++)
{
...
float2 prevWeights = float2(-6 + 2*k*k, -k*k + 3 * k - 3) / denominator;
#line 318
prev[0] = UnpackColors(tex2Dfetch(sBlur0, clamp(float2(coord.x, coord.y + filterWidth), 0, float2(3840, 2137) - 1))).xyzx;
prev[1] = prev[0].yyyy;
prev[2] = prev[0].zzzz;
prev[0] = prev[0].xxxx;
curr[0] = prev[0];
...
DispatchSizeX = uint2(uint(((3840) + (256) - 1) / (256)), uint(((2137) + (64) - 1) / (64))).x;
DispatchSizeY = uint2(uint(((3840) + (256) - 1) / (256)), uint(((2137) + (64) - 1) / (64))).y;
}
#line 458
pass
{
...
DispatchSizeX = uint2(uint(((3840) + (64) - 1) / (64)), uint(((2137) + (256) - 1) / (256))).x;
DispatchSizeY = uint2(uint(((3840) + (64) - 1) / (64)), uint(((2137) + (256) - 1) / (256))).y;
}
#line 472
pass
{
