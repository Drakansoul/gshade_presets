...
> = false;
#line 13
texture chromaSubTex { Width = 3840; Height = 2137; Format = RGBA8; };
...
if(chromaSubType == 0)
{
color += tex2D(chromaSubSamp, float2(texcoord.x + (1.0 / 3840), texcoord.y)).rgb;
}
else if(chromaSubType == 1)
{
color += tex2D(chromaSubSamp, float2(texcoord.x + (1.0 / 3840), texcoord.y)).rgb;
color += tex2D(chromaSubSamp, float2(texcoord.x, texcoord.y + (1.0 / 2137))).rgb;
color += tex2D(chromaSubSamp, float2(texcoord.x + (1.0 / 3840), texcoord.y + (1.0 / 2137))).rgb;
}
else if(chromaSubType == 2)
{
color += tex2D(chromaSubSamp, float2(texcoord.x + (1.0 / 3840), texcoord.y)).rgb;
color += tex2D(chromaSubSamp, float2(texcoord.x + (1.0 / 3840) * 2, texcoord.y)).rgb;
color += tex2D(chromaSubSamp, float2(texcoord.x + (1.0 / 3840) * 3, texcoord.y)).rgb;
}
else
{
color += tex2D(chromaSubSamp, float2(texcoord.x + (1.0 / 3840), texcoord.y)).rgb;
