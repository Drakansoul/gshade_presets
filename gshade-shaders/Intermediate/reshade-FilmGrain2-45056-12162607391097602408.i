...
#line 1 "C:\Program Files\GShade\gshade-shaders\Shaders\FilmGrain2.fx"
#line 11
...
#line 34 "C:\Program Files\GShade\gshade-shaders\Shaders\FilmGrain2.fx"
#line 35
...
{
float rotX = ((tc.x * 2.0 - 1.0) * (3840 * (1.0 / 2137)) * cos(angle)) - ((tc.y * 2.0 - 1.0) * sin(angle));
float rotY = ((tc.y * 2.0 - 1.0) * cos(angle)) + ((tc.x * 2.0 - 1.0) * (3840 * (1.0 / 2137)) * sin(angle));
rotX = ((rotX / (3840 * (1.0 / 2137))) * 0.5 + 0.5);
rotY = rotY * 0.5 + 0.5;
#line 113
...
float3 noise = pnoise3D(float3(rotCoordsR * float2(3840, 2137) / grainsize, 0.0)).xxx;
#line 122
if (coloramount > 0)
{
float2 rotCoordsG = coordRot(texCoord, timer + rotOffset.y);
float2 rotCoordsB = coordRot(texCoord, timer + rotOffset.z);
noise.g = lerp(noise.r, pnoise3D(float3(rotCoordsG * float2(3840, 2137) / grainsize, 1.0)), coloramount);
noise.b = lerp(noise.r, pnoise3D(float3(rotCoordsB * float2(3840, 2137) / grainsize, 2.0)), coloramount);
}
#line 130
