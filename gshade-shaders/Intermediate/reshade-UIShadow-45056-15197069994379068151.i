...
> = 2;
#line 62
texture2D shadowTex < pooled = true; > { Width = 3840; Height = 2137; Format = R8; };
texture2D shadowTexPong { Width = 3840; Height = 2137; Format = R8; };
#line 65
...
const int2 pixelCoord = texcoord * float2(3840, 2137);
#line 132
const float4 ul = float4(tex2DgatherA(ReShade::BackBuffer, texcoord - 0.5 * float2((1.0 / 3840), (1.0 / 2137))).rba, tex2Dfetch(ReShade::BackBuffer, pixelCoord + int2(-1, 1)).a);
const float4 lr = float4(tex2DgatherA(ReShade::BackBuffer, texcoord + 0.5 * float2((1.0 / 3840), (1.0 / 2137))).rgb, tex2Dfetch(ReShade::BackBuffer, pixelCoord + int2( 1,-1)).a);
#line 136
...
{
float4 tmp = tex2DgatherR(shadowSampler, texcoord - 0.5 * float2((1.0 / 3840), (1.0 / 2137)));
float output = max(max(tmp.r, tmp.g), max(tmp.b, tmp.a));
tmp = tex2DgatherR(shadowSampler, texcoord + 0.5 * float2((1.0 / 3840), (1.0 / 2137)));
output = max(max(max(tmp.r, tmp.g), max(tmp.b, tmp.a)), output);
tmp = tex2DgatherR(shadowSampler, texcoord + float2(0.5, -0.5) * float2((1.0 / 3840), (1.0 / 2137)));
output = max(max(max(tmp.r, tmp.g), max(tmp.b, tmp.a)), output);
tmp = tex2DgatherR(shadowSampler, texcoord + float2(-0.5, 0.5) * float2((1.0 / 3840), (1.0 / 2137)));
output = max(max(max(tmp.r, tmp.g), max(tmp.b, tmp.a)), output);
#line 168
...
{
float4 tmp = tex2DgatherR(shadowPongSampler, texcoord - 0.5 * float2((1.0 / 3840), (1.0 / 2137)));
float output = max(max(tmp.r, tmp.g), max(tmp.b, tmp.a));
tmp = tex2DgatherR(shadowPongSampler, texcoord + 0.5 * float2((1.0 / 3840), (1.0 / 2137)));
output = max(max(max(tmp.r, tmp.g), max(tmp.b, tmp.a)), output);
tmp = tex2DgatherR(shadowPongSampler, texcoord + float2(0.5, -0.5) * float2((1.0 / 3840), (1.0 / 2137)));
output = max(max(max(tmp.r, tmp.g), max(tmp.b, tmp.a)), output);
tmp = tex2DgatherR(shadowPongSampler, texcoord + float2(-0.5, 0.5) * float2((1.0 / 3840), (1.0 / 2137)));
output = max(max(max(tmp.r, tmp.g), max(tmp.b, tmp.a)), output);
#line 182
...
{
float output = dot(tex2DgatherR(shadowPongSampler, texcoord - 0.5 * float2((1.0 / 3840), (1.0 / 2137))), float4(1,1,1,1));
output += dot(tex2DgatherR(shadowPongSampler, texcoord + 0.5 * float2((1.0 / 3840), (1.0 / 2137))), float4(1,1,1,1));
output += dot(tex2DgatherR(shadowPongSampler, texcoord + float2(0.5, -0.5) * float2((1.0 / 3840), (1.0 / 2137))), float4(1,1,1,1));
output += dot(tex2DgatherR(shadowPongSampler, texcoord + float2(-0.5, 0.5) * float2((1.0 / 3840), (1.0 / 2137))), float4(1,1,1,1));
#line 196
...
{
float output = dot(tex2DgatherR(shadowSampler, texcoord - 0.5 * float2((1.0 / 3840), (1.0 / 2137))), float4(1,1,1,1));
output += dot(tex2DgatherR(shadowSampler, texcoord + 0.5 * float2((1.0 / 3840), (1.0 / 2137))), float4(1,1,1,1));
output += dot(tex2DgatherR(shadowSampler, texcoord + float2(0.5, -0.5) * float2((1.0 / 3840), (1.0 / 2137))), float4(1,1,1,1));
output += dot(tex2DgatherR(shadowSampler, texcoord + float2(-0.5, 0.5) * float2((1.0 / 3840), (1.0 / 2137))), float4(1,1,1,1));
#line 207
return float4(output * 0.0625, 0, 0, 0);
}
