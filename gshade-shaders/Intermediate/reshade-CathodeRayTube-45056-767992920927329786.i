...
> = true;
#line 13
texture AGSUt { Width = 3840; Height = 2137; Format = RGBA16F; };
sampler AGSUs { Texture = AGSUt; AddressU = BORDER; AddressV = BORDER; };
#line 16
texture AGt { Width = 3840; Height = 2137; Format = RGBA16F; };
sampler AGs { Texture = AGt; AddressU = BORDER; AddressV = BORDER; };
#line 19
texture SMSUt { Width = 3840; Height = 2137; Format = RGBA16F; };
sampler SMSUs { Texture = SMSUt; AddressU = BORDER; AddressV = BORDER; };
#line 22
texture SMt { Width = 3840; Height = 2137; Format = RGBA16F; };
sampler SMs { Texture = SMt; AddressU = BORDER; AddressV = BORDER; };
#line 25
texture ShMt { Width = 3840; Height = 2137; Format = RGBA16F; };
...
{
blur += tex2Dlod(ReShade::BackBuffer, float4(texcoord1.x + ((1.0 / 3840) * x), texcoord1.y + ((1.0 / 2137) * y), 0.0, 0.0)).rgb;
divisor++;
}
}
#line 42
...
+ tex2D(AGSUs, float2(texcoord.x + (1.0 / 3840), texcoord.y)).rgb
+ tex2D(AGSUs, float2(texcoord.x - (1.0 / 3840), texcoord.y)).rgb;
#line 68
...
+ tex2D(SMSUs, float2(texcoord.x + (1.0 / 3840), texcoord.y)).rgb
+ tex2D(SMSUs, float2(texcoord.x - (1.0 / 3840), texcoord.y)).rgb;
#line 107
...
+ tex2D(AGs, float2(texcoord.x, texcoord.y + (1.0 / 2137))).rgb
+ tex2D(AGs, float2(texcoord.x, texcoord.y - (1.0 / 2137))).rgb;
}
else if(mask == 1)
{
return tex2D(SMs, texcoord).rgb
+ tex2D(SMs, float2(texcoord.x, texcoord.y + (1.0 / 2137))).rgb
+ tex2D(SMs, float2(texcoord.x, texcoord.y - (1.0 / 2137))).rgb;
}
else
{
...
{
return tex2D(ShMs, texcoord).rgb
+ tex2D(ShMs, float2(texcoord.x + (1.0 / 3840), texcoord.y)).rgb
+ tex2D(ShMs, float2(texcoord.x - (1.0 / 3840), texcoord.y)).rgb
+ tex2D(ShMs, float2(texcoord.x, texcoord.y + (1.0 / 2137))).rgb
+ tex2D(ShMs, float2(texcoord.x, texcoord.y - (1.0 / 2137))).rgb
+ tex2D(ShMs, float2(texcoord.x - ((1.0 / 3840) * 2), texcoord.y)).rgb
+ tex2D(ShMs, float2(texcoord.x, texcoord.y - ((1.0 / 2137) * 2))).rgb
+ tex2D(ShMs, float2(texcoord.x + (1.0 / 3840), texcoord.y - (1.0 / 2137))).rgb
+ tex2D(ShMs, float2(texcoord.x - (1.0 / 3840), texcoord.y + (1.0 / 2137))).rgb
+ tex2D(ShMs, float2(texcoord.x - (1.0 / 3840), texcoord.y - (1.0 / 2137))).rgb
+ tex2D(ShMs, float2(texcoord.x - (1.0 / 3840), texcoord.y - ((1.0 / 2137) * 2))).rgb
+ tex2D(ShMs, float2(texcoord.x - ((1.0 / 3840) * 2), texcoord.y - (1.0 / 2137))).rgb;
}
}
#line 259
return 0;
