...
texture Temp0 <Pooled = true;>{Width = 3840; Height = 2137; Format = RGB10A2;};
texture Temp1 <Pooled = true;>{Width = 3840 * 2; Height = 2137; Format = RGB10A2;};
#line 50
...
{
if(id.x < (uint2((uint(((3840) + (128) - 1) / (128)) * 2), uint(((2137) + (64) - 1) / (64))).x / 2))
{
...
{
float2 coord = float2((id.x - (uint2((uint(((3840) + (128) - 1) / (128)) * 2), uint(((2137) + (64) - 1) / (64))).x / 2)) * 128 + 128, id.y) + 0.5;
float3 curr;
float3 prev;
float3 weight;
prev = tex2Dfetch(sBackBuffer, float2(coord.x + 32, coord.y)).xyz;
...
if(i <= 0)
{
tex2Dstore(wTemp1, int2(3840 + coord.x + i, coord.y), prev.xyzx);
}
}
}
}
...
{
if(id.y < uint2(uint(((3840) + (64) - 1) / (64)), (uint(((2137) + (128) - 1) / (128)) * 2)).y / 2)
{
...
{
float2 coord = float2(id.x, (id.y - (uint2(uint(((3840) + (64) - 1) / (64)), (uint(((2137) + (128) - 1) / (128)) * 2)).y / 2)) * 128 + 128) + 0.5;
float3 curr;
float3 prev;
float3 weight;
prev = tex2Dfetch(sTemp0, float2(coord.x, coord.y + 32)).xyz;
...
if(i <= 0)
{
tex2Dstore(wTemp1, int2(3840 + coord.x, coord.y + i), prev.xyzx);
}
}
}
}
...
DispatchSizeX = uint2((uint(((3840) + (128) - 1) / (128)) * 2), uint(((2137) + (64) - 1) / (64))).x;
DispatchSizeY = uint2((uint(((3840) + (128) - 1) / (128)) * 2), uint(((2137) + (64) - 1) / (64))).y;
}
#line 200
pass
{
...
DispatchSizeX = uint2(uint(((3840) + (64) - 1) / (64)), (uint(((2137) + (128) - 1) / (128)) * 2)).x;
DispatchSizeY = uint2(uint(((3840) + (64) - 1) / (64)), (uint(((2137) + (128) - 1) / (128)) * 2)).y;
}
#line 215
pass
{
